...
```text
请按以下流程操作：
1. 基于 https://github.com/ShuTianLe/edgeever 创建 GitHub 仓库并克隆到本地。
...
> 本分支不启用 R2，附件使用 Workers Free 自带的 KV。KV 达到免费账号硬配额后操作会失败，不会产生超额费用；请勿把账号升级到 Workers Paid。
...
推荐优先使用自动化辅助命令。手动配置模板使用 `admin` / `admin123` 作为初始登录账号密码，之后可在个人设置中修改密码。如果选择完全手动创建 Cloudflare 资源，必须先在 `.env.local` 中完成 D1 ID、Workers KV namespace ID 和 400 天会话期限等配置，再执行 `bun run deploy:manual`。
...
- 后端：Cloudflare Workers、Hono、Zod、D1、Workers KV，提供 REST API、OpenAPI 与 Remote MCP。
...
启动默认开发环境。它会先应用本地迁移，并在首次启动时使用仓库内固定的 Demo 种子初始化本地 D1/KV；后续重启会保留本地修改，且不会连接任何远程实例。
...
wrangler.toml     Cloudflare Workers、Assets、D1、Workers KV 配置
```
