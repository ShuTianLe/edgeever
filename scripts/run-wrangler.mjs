...
const kvNamespaceId = envValue("KV_NAMESPACE_ID");
if (kvNamespaceId) {
  if (!KV_ID_PATTERN.test(kvNamespaceId)) {
    throw new Error("EDGE_EVER_KV_NAMESPACE_ID must be a 32-character Cloudflare KV namespace ID.");
  }
  config = replaceTomlValue(config, "id", kvNamespaceId);
}

const kvPreviewNamespaceId = envValue("KV_PREVIEW_NAMESPACE_ID");
if (kvPreviewNamespaceId) {
  if (!KV_ID_PATTERN.test(kvPreviewNamespaceId)) {
    throw new Error("EDGE_EVER_KV_PREVIEW_NAMESPACE_ID must be a 32-character Cloudflare KV namespace ID.");
  }
  config = replaceTomlValue(config, "preview_id", kvPreviewNamespaceId);
}

const runtimeVars = {
  EDGE_EVER_AUTH_USERNAME: envValue("AUTH_USERNAME"),
  EDGE_EVER_SESSION_TTL_DAYS: envValue("SESSION_TTL_DAYS"),
  EDGE_EVER_KV_NAMESPACE_NAME: envValue("KV_NAMESPACE_NAME"),
  EDGE_EVER_RESOURCE_STORAGE_LIMIT_BYTES: envValue("RESOURCE_STORAGE_LIMIT_BYTES"),
  EDGE_EVER_RESOURCE_GC_CRON: envValue("RESOURCE_GC_CRON"),
...
const resourceGcCron = envValue("RESOURCE_GC_CRON") || "15 0 * * *";
const crons = [resourceGcCron];
if (demoMode === "true") {
  const demoResetCron = envValue("DEMO_RESET_CRON") || "0 19 * * *";
  if (!crons.includes(demoResetCron)) {
    crons.push(demoResetCron);
  }
}

const cronPattern = /^crons\s*=\s*\[[^\n]*\]/m;
if (!cronPattern.test(config)) {
  throw new Error(`Cannot find triggers.crons in ${baseConfigPath}`);
}
changed = true;
config = config.replace(cronPattern, `crons = [${crons.map(tomlString).join(", ")}]`);
...
if (isRemoteCommand && config.includes(`id = "${PLACEHOLDER_KV_ID}"`)) {
  console.error(
    [
      "Missing Cloudflare KV namespace id.",
      instanceKey
        ? `Set EDGE_EVER_${instanceKey}_KV_NAMESPACE_ID or EDGE_EVER_KV_NAMESPACE_ID,`
        : "Set EDGE_EVER_KV_NAMESPACE_ID,",
      "or replace the id placeholder in wrangler.toml / WRANGLER_CONFIG.",
    ].join(" "),
  );
  process.exit(1);
}

if (isRemoteCommand && config.includes(`preview_id = "${PLACEHOLDER_KV_ID}"`)) {
  console.error(
    [
      "Missing Cloudflare KV preview namespace id.",
      instanceKey
        ? `Set EDGE_EVER_${instanceKey}_KV_PREVIEW_NAMESPACE_ID or EDGE_EVER_KV_PREVIEW_NAMESPACE_ID,`
        : "Set EDGE_EVER_KV_PREVIEW_NAMESPACE_ID,",
      "or replace the preview_id placeholder in wrangler.toml / WRANGLER_CONFIG.",
    ].join(" "),
  );
  process.exit(1);
}
...
const isDryRun = wranglerArgs.includes("--dry-run");
...
if (result.status === 0 && isDeployCommand && !isDryRun) {
